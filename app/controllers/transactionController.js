    ...
    if (!result) {
      ...
      if (isSoldSeatExists.indexOf(true) !== -1) {
        ...
        return response(
          ...
          400,
          false,
          "Sorry the selected seat has been used"
        );
      }
      try {
        const result = await SoldSeats.create(
          req.body.showTimeId,
...
exports.getUserOrderHistoryDetail = async (req, res) => {
  try {
    const results = await Transactions.getOrderHistoryDetail(
      req.params.id,
      req.data.id
    );
    ...
    if (results.length < 1) {
      ...
      const orderHistory = {
        ...
        cinemaPoster:
          process.env.APP_URL + "/public/cinemas/" + results[0].cinemaPoster,
      };
      return response(
        res,
        200,
