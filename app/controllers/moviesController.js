...
exports.createMovie = async (req, res) => {
  try {
    ...
    if (typeof picture === "object" && picture.success === false)
...
exports.getMovies = async (req, res) => {
  ...
  try {
    ...
    const nextMovies = await Movies.getAllMovies(
      limit,
      offset + dataLimit,
      search,
      order,
    ...
    const query = queryKey
      ...
      .join("&");
    const totalPage = Math.ceil(allMovie.length / Number(limit));
    const nextPageLink =
      nextMovies.length > 0
...
exports.getMovieById = async (req, res) => {
  ...
  try {
    ...
    if (results.length < 1)
      return response(res, 400, false, `Movie with id: ${id} is not exists`);
  ...
  try {
    ...
    if (results.length < 1)
      return response(
        ...
        400,
        false,
        `Movie with month: ${month} is not exists`
      );
  ...
  try {
    ...
    if (results.affectedRows > 0) {
      ...
      return response(
        ...
        200,
        true,
        `Movie with id: ${id} successfully edited`,
...
exports.deleteMovie = async (req, res) => {
  ...
  try {
    ...
    return response(
      ...
      200,
      true,
      `Movie with id: ${id} successfully deleted`,
      movie[0]
    );
  } catch (error) {
    return response(res, 500, false, error.message);
